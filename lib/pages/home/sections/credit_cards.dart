...
class CreditCardAccounts extends ConsumerWidget {
    ...
    return Column(
      ...
      children: [
        for (final (index, account) in accounts.indexed) ...[
          _buildAccountCard(account: account, money: money),
          const SizedBox(height: 12),
          if (index != accounts.length - 1) ...[
            const Divider(height: 12, color: AppColors.cardOutline),
            const SizedBox(height: 12),
          ],
        ],
      ],
  ...
  Widget _buildAccountCard({required CreditAccount account, required Money money}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          ...
          children: [
            ...
            Text(money.formatPercent(account.balance, account.limit), style: AppTextStyles.cardHeadlineStyle),
          ],
        ),
        ...
        Row(
          ...
          children: [
            Row(
              children: [
                Text(money.formatDollars(account.balance), style: AppTextStyles.cardHeadlineLightStyle),
            ...
            Row(
              children: [
                Text(money.formatDollars(account.limit), style: AppTextStyles.cardHeadlineLightStyle),
                const SizedBox(width: 8),
